...
const Tweets = () => {
  ...
  useEffect(() => {
    const fetchTweets = async () => {
      try {
        const response = await axios.get('http://localhost:5050/tweets');
  ...
  const handleTweetSubmit = async () => {
    try {
      const response = await axios.post('http://localhost:5050/tweets', {
        text: tweetText,
      ...
      if (response.status === 201) {
        ...
        const tweetsResponse = await axios.get('http://localhost:5050/tweets');
